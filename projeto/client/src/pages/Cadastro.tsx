...
import { PlatformConfigPanel } from "../components/Studio/PlatformConfigPanel";
...
export default function Cadastro() {
  ...
  const loadTenantStorageConfig = async (tenantSlug: string) => {
    ...
    if (data?.config && typeof data.config === "object") {
      setStorageConfig(data.config as Record<string, string>);
    } else {
      setStorageConfig({});
    }
  };
  ...
  const saveTenantStorageConfig = async () => {
    ...
    try {
      const { error } = await supabase
        .from("tenant_storage_config")
        .upsert(
          { tenant_slug: storageTenant, provider: storageProvider, config: storageConfig },
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 p-8 font-sans">
        ...
        <div className="mt-14 border-t border-zinc-800 pt-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-zinc-500">Escolha o provider e configure as credenciais de acesso para cada cliente.</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-12 gap-8">
            ...
            <div className="col-span-12 lg:col-span-8 space-y-5">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-zinc-500 uppercase">Provider</label>
                <Select value={storageProvider} onValueChange={(v) => { setStorageProvider(v); setStorageConfig({}); }}>
                  <SelectTrigger className="bg-black border-zinc-800 text-white">
                    <SelectValue placeholder="Provider" />
                  </SelectTrigger>
                  <SelectContent className="bg-zinc-900 border-zinc-700 text-white">
              ...
              {storageProvider === "gdrive" && (
                <div className="space-y-3 p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                  <p className="text-[10px] text-zinc-500 leading-relaxed">
                    Clique em <strong className="text-zinc-300">Conectar Google Drive</strong> para autorizar via OAuth.
                    Após autorizar no popup, cole o <strong className="text-zinc-300">ID da pasta</strong> onde os arquivos serão salvos (opcional — deixe em branco para usar a raiz).
                  </p>
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase">ID da Pasta do Drive (opcional)</label>
                    <Input
                      className="bg-black border-zinc-700 text-white"
                      placeholder="Ex: 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgVE2upms"
                      value={storageConfig.folder_id || ""}
                      onChange={(e) => setStorageConfig({ ...storageConfig, folder_id: e.target.value })}
                    />
                    <p className="text-[10px] text-zinc-600">
                      Encontre o ID na URL do Drive: drive.google.com/drive/folders/<strong>ID_AQUI</strong>
                    </p>
                  </div>
                  <Button
                  ...
                  <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
                    ...
                    <button onClick={refreshDriveStatus} className="text-zinc-500 hover:text-white font-bold normal-case">
                      Atualizar
                    </button>
                    {driveFolderId && <span className="text-zinc-600 normal-case font-normal">({driveFolderId})</span>}
                  </div>
                </div>
              )}
              ...
              {storageProvider === "r2" && (
                <div className="space-y-3 p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                  <p className="text-[10px] text-zinc-500 leading-relaxed">
                    Configure seu bucket Cloudflare R2. Encontre as credenciais em <strong className="text-zinc-300">Cloudflare Dashboard → R2 → Manage API Tokens</strong>.
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Account ID</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="a1b2c3d4e5f6..." value={storageConfig.account_id || ""} onChange={(e) => setStorageConfig({ ...storageConfig, account_id: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Bucket Name</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="meu-bucket-r2" value={storageConfig.bucket || ""} onChange={(e) => setStorageConfig({ ...storageConfig, bucket: e.target.value })} />
                    </div>
                    <div className="col-span-2 space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Endpoint URL</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="https://<account_id>.r2.cloudflarestorage.com" value={storageConfig.endpoint || ""} onChange={(e) => setStorageConfig({ ...storageConfig, endpoint: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Access Key ID</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="Access Key ID" value={storageConfig.access_key_id || ""} onChange={(e) => setStorageConfig({ ...storageConfig, access_key_id: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Secret Access Key</label>
                      <Input type="password" className="bg-black border-zinc-700 text-white text-xs" placeholder="••••••••••••" value={storageConfig.secret_key || ""} onChange={(e) => setStorageConfig({ ...storageConfig, secret_key: e.target.value })} />
                    </div>
                  </div>
                </div>
              )}
              ...
              {storageProvider === "s3" && (
                <div className="space-y-3 p-4 bg-zinc-900/50 border border-zinc-800 rounded-xl">
                  <p className="text-[10px] text-zinc-500 leading-relaxed">
                    Configure seu bucket AWS S3. Encontre as credenciais em <strong className="text-zinc-300">AWS Console → IAM → Users → Security Credentials</strong>.
                  </p>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Bucket Name</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="meu-bucket-s3" value={storageConfig.bucket || ""} onChange={(e) => setStorageConfig({ ...storageConfig, bucket: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Region</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="us-east-1" value={storageConfig.region || ""} onChange={(e) => setStorageConfig({ ...storageConfig, region: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Access Key ID</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="AKIAIOSFODNN7EXAMPLE" value={storageConfig.access_key_id || ""} onChange={(e) => setStorageConfig({ ...storageConfig, access_key_id: e.target.value })} />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Secret Access Key</label>
                      <Input type="password" className="bg-black border-zinc-700 text-white text-xs" placeholder="••••••••••••" value={storageConfig.secret_key || ""} onChange={(e) => setStorageConfig({ ...storageConfig, secret_key: e.target.value })} />
                    ...
                    <div className="col-span-2 space-y-1.5">
                      <label className="text-[10px] font-bold text-zinc-500 uppercase">Endpoint personalizado (opcional)</label>
                      <Input className="bg-black border-zinc-700 text-white text-xs" placeholder="https://s3.amazonaws.com (deixe vazio para padrão AWS)" value={storageConfig.endpoint || ""} onChange={(e) => setStorageConfig({ ...storageConfig, endpoint: e.target.value })} />
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-end gap-3">
                <Button
                  ...
                  disabled={storageLoading || !storageTenant}
                  className="bg-blue-600 hover:bg-blue-500 text-white font-bold"
                >
                  {storageLoading ? "Salvando..." : `Salvar ${storageProvider === "gdrive" ? "Google Drive" : storageProvider === "r2" ? "Cloudflare R2" : "AWS S3"}`}
                </Button>
              </div>
            </div>
          </div>
        ...
        {/* ====== PLATAFORMAS DE IA POR TENANT ====== */}
        <div className="mt-14 border-t border-zinc-800 pt-8">
          <PlatformConfigPanel tenantSlug={apiTenant || storageTenant || ""} />
        </div>
