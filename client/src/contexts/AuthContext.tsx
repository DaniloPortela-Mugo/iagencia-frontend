...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    let userProfileLoaded = false; // flag separada: perfil já foi carregado nesta sessão
    ...
    const init = async () => {
      try {
        ...
        if (data?.session?.user) {
          ...
          if (!userProfileLoaded && !profileLoadInProgress) {
            profileLoadInProgress = true;
            try { await loadProfile(data.session.user.id, data.session.user.email ?? null); }
            finally { profileLoadInProgress = false; }
        ...
        if (cached?.user?.id) {
          ...
          if (!userProfileLoaded && !profileLoadInProgress) {
            profileLoadInProgress = true;
            try { await loadProfile(cached.user.id, cached.user.email ?? null); }
            finally { profileLoadInProgress = false; }
            userProfileLoaded = true;
          }
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "TOKEN_REFRESHED") {
        if (session?.user) {
          ...
          if (!userProfileLoaded && !profileLoadInProgress) {
            profileLoadInProgress = true;
            try { await loadProfile(session.user.id, session.user.email ?? null); }
            finally { profileLoadInProgress = false; }
            userProfileLoaded = true;
            finishInit();
          }
      ...
      if (session?.user) {
        setToken(session.access_token || null);
        // Carrega perfil apenas se: não está em andamento E ainda não foi carregado
        if (!profileLoadInProgress && !userProfileLoaded) {
          profileLoadInProgress = true;
          try { await loadProfile(session.user.id, session.user.email ?? null); }
          finally { profileLoadInProgress = false; }
          userProfileLoaded = true;
