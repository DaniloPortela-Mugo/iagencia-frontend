...
const stats = useMemo(() => {
  ...
  useEffect(() => {
    ...
    const fetchCredits = async () => {
      try {
        ...
        const [walletsRes, tenantsRes, logsRes, logsMonthRes] = await Promise.all([
          walletQuery,
          tenantsQuery,
          logsQuery,
          logsMonthQuery,
        ...
        if (logsMonthRes.error) throw logsMonthRes.error;
  ...
  // GESTÃO DE CAPACIDADE (Workload)
  const TEAM_METRICS = [
    { id: 1, name: "Danilo Portela", role: "Dir. Arte", load: (stats.byDept.arte * 15), dept: 'arte' },
    { id: 2, name: "Mônica M.", role: "Redação", load: (stats.byDept.redacao * 20), dept: 'redacao' },
    { id: 3, name: "Rodrigo O.", role: "RTV / Vídeo", load: (stats.byDept.producao * 25), dept: 'producao' },
    { id: 4, name: "Julia P.", role: "Atendimento", load: (stats.critical * 10), dept: 'atendimento' }
  ];
